...
def map_symbols_to_gene_ids(df, db_path):
    ...
    mapped = pd.merge(df, mapping_df, on='Gene', how='inner')

    return mapped


def map_gene_ids_to_symbols(df, db_path, id_column='geneID_short'):
    conn = sqlite3.connect(db_path)
    mapping_query = "SELECT ensembl_gene_id as geneID_mapping, symbol FROM genes"
    mapping_df = pd.read_sql(mapping_query, conn)
    conn.close()
...
def cis_trans(hits, cnv_col='CNV'):
    ...
    cnv_over_cis = (over) & (hits[cnv_col].str.contains('AMP|DUP', case=False, na=False))
    cnv_under_cis = (under) & (hits[cnv_col].str.contains('DEL', case=False, na=False))
...
def find_trans_drivers(df, cnv_col='CNV', join_mech='left'):
    trans = df[df['Mechanism'] == 'trans effect'][[sample_id, 'geneID_short', 'zScore']]
    trans.columns = [sample_id, 'target_gene', 'target_zScore']

    cnv_mask = (df[cnv_col].notna()) & (df[cnv_col] != 'No CNV')
    cnv = df[cnv_mask][[sample_id, 'geneID_filtered', cnv_col]]

    cnv.columns = [sample_id, 'driver_gene', 'driver_CNV']

    # maybe use different method if too slow
    drivers = pd.merge(trans, cnv, on=sample_id, how=join_mech)
    # for safety:
    drivers = drivers.dropna(subset=['driver_gene', 'driver_CNV'])
